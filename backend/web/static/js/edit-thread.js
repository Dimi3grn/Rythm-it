...
document.addEventListener('DOMContentLoaded', function() {
    console.log('🚀 Edit Thread JS chargé');
...
function updateImageDisplay(imageUrl) {
    ...
    imageSection.innerHTML = `
        <div class="current-image">
            <img src="${imageUrl}" alt="Image du thread" class="current-image-preview">
...
function showUploadProgress() {
    const placeholder = document.querySelector('.upload-placeholder, .current-image');
    if (placeholder) {
...
function loadAvailableTags() {
    ...
    .then(data => {
        if (data.success && data.tags) {
            availableTags = data.tags.map(tag => ({
                name: tag.name,
                type: tag.type
            }));
        }
